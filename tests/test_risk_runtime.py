...
# The one outstanding charge in the fixture is a fixed and floating charge
# over everything the company has, so it scores as a debenture too.
DEBENTURE = "an all-assets debenture is outstanding (trade creditors rank behind)"
...
async def test_sensor_state_attributes_and_store(
    ...
    # The one outstanding charge (2) is an all-assets debenture (3).
    assert attrs["score"] == 5
    assert attrs["reason"] == f"Green: {DEBENTURE}; 1 outstanding charge"
    assert attrs["reasons"] == [DEBENTURE, "1 outstanding charge"]
    ...
    assert stored[ACTIVE]["risk_score"] == 5
...
async def test_score_changes_are_remembered_without_an_event(
    ...
    await hass.async_block_till_done()
    assert hass.states.get(SENSOR).state == "green"
    assert hass.states.get(SENSOR).attributes["score"] == 5  # still 1 to 2 charges
    assert company.state.risk_score == 5
    ...
    assert company.state.risk_score == 13
...
async def test_a_profile_that_will_not_refresh_goes_amber_after_a_week(
    ...
    assert state.attributes["reasons"] == [DEBENTURE, "1 outstanding charge"]
