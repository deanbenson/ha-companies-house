...
# Everything that ends or holds off a strike-off, notice or application
# alike: the same lists the probe reads, so the two never disagree.
STRIKE_OFF_ENDED: Final = (
    STRIKE_OFF_DISCONTINUED_DESCRIPTIONS | STRIKE_OFF_SUSPENDED_DESCRIPTIONS
)
...
class _Scorer:
    ...
    def _strike_off(self) -> None:
        ...
        # Whether a strike-off is live is the probe's call, the same one the
        # countdown sensors make: the register's status detail, or the notice
        # it remembered, and neither once a filing discontinued it (the status
        # detail follows that filing days later, or never) or the register
        # cleared it. The filings only fill in the kind and the dates.
        proposed = self.state.strike_off_discontinued_on is None and (
            self.profile.company_status_detail == STATUS_DETAIL_STRIKE_OFF
            or self.state.strike_off_notice_on is not None
        )
        if proposed:
            kind = self.state.strike_off_kind or (
                STRIKE_OFF_NOTICES.get(notice.description or "") if notice else None
            )
            ...
            suspended_on = self.state.strike_off_suspended_on
            if suspended_on is None and (
                ended is not None
                and not notice_live
                and ended.description in STRIKE_OFF_SUSPENDED_DESCRIPTIONS
            ):
                suspended_on = ended.date
            if suspended_on is not None:
                text += f", suspended on {_pretty_date(suspended_on)}"
            self.add("R4", text)
            return
